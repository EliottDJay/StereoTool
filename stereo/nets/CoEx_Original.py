...
from stereo.modules.aggregation import GCE_Original
...
class CoEx_Original(nn.Module):
    def __init__(self, args):
        ...
        self.feature = BasicFeature(backbone)
        self.up = BasicFeaUp(backbone)
        originalc = backbone['channels']
        ...
        self.aggregation = GCE_Original(args, self.D//4, matching_head=volumeset['group'], im_channels=fea_c_up)
    ...
    def forward(self, left_img, right_img):
        ...
        disp_pred_x4 = regression_topk(volume, self.topk, self.D // 4)  # 4D volume
        if self.training:
