...
def poolfeat_head(input, prob, sp_h=2, sp_w=2):
    ...
    sum_all = F.avg_pool2d(sum_all.reshape(b, -1, h, w), kernel_size=(sp_h, sp_w), stride=(sp_h, sp_w)).reshape(b, -1,
                                                                                                                c,
                                                                                                                h // sp_h,
                                                                                                                w // sp_w)
    pooled_feat = sum_all[:, :-1] / (sum_all[:, -1:] + 1e-8)
...
def poolfeat3d(input, prob, sp_h=2, sp_w=2):
    ...
    sum_all = F.avg_pool2d(sum_all, kernel_size=(sp_h, sp_w), stride=(sp_h, sp_w)).reshape(b, -1, d + 1, h // sp_h,
                                                                                           w // sp_w)
    pooled_feat = sum_all[:, :, :-1] / (sum_all[:, :, -1:] + 1e-8)
...
def upfeat(input, prob, up_h=4, up_w=4):
    """

    :param input: 4D disp map so,when input a 3D disp map we should use unsqueese function
    :param prob:
    :param up_h:
    :param up_w:
    :return: 4D disp map, we need squeeze function some time
    """
    b, c, h, w = input.shape
